...
class Pagination {
    ...
    public static function navigation($page, $total, $total_page, $num) {
        ...
        if ($page != 1) {
            $previewpage = '<a href="?page=1' . $cat . '" class="pagination__link">Первая</a><a href="?page=' . ($page - 1) . $cat . '" class="pagination__link">Предидущая</a> ';
        } else {
            $previewpage = '';
        }
        
        if ($page != $total_page) {
            $nextpage = '<a href="?page=' . ($page + 1) . $cat . '" class="pagination__link">Следующая</a><a href="?page=' . $total_page . $cat . '" class="pagination__link">Последняя</a>';
        } else {
            $nextpage = '';
        }
       
        if ($page - 5 > 0) {
            $page5left = ' <a href="?page=' . ($page - 5) . $cat . '" class="pagination__link">' . ($page - 5) . '</a> ';
        } else {
            $page5left = '';
        }
        if ($page - 4 > 0) {
            $page4left = ' <a href="?page=' . ($page - 4) . $cat . '" class="pagination__link">' . ($page - 4) . '</a> ';
        } else {
            $page4left = '';
        }
        if ($page - 3 > 0) {
            $page3left = ' <a href="?page=' . ($page - 3) . $cat . '" class="pagination__link">' . ($page - 3) . '</a> ';
        } else {
            $page3left = '';
        }
        if ($page - 2 > 0) {
            $page2left = ' <a href="?page=' . ($page - 2) . $cat . '" class="pagination__link">' . ($page - 2) . '</a> ';
        } else {
            $page2left = '';
        }
        if ($page - 1 > 0) {
            $page1left = ' <a href="?page=' . ($page - 1) . $cat . '" class="pagination__link">' . ($page - 1) . '</a> ';
        } else {
            $page1left = '';
        }
        if ($page + 5 <= $total_page) {
            $page5right = ' <a href="?page=' . ($page + 5) . $cat . '" class="pagination__link">' . ($page + 5) . '</a>';
        } else {
            $page5right = '';
        }
        if ($page + 4 <= $total_page) {
            $page4right = ' <a href="?page=' . ($page + 4) . $cat . '" class="pagination__link">' . ($page + 4) . '</a>';
        } else {
            $page4right = '';
        }
        if ($page + 3 <= $total_page) {
            $page3right = ' <a href="?page=' . ($page + 3) . $cat . '" class="pagination__link">' . ($page + 3) . '</a>';
        } else {
            $page3right = '';
        }
        if ($page + 2 <= $total_page) {
            $page2right = ' <a href="?page=' . ($page + 2) . $cat . '" class="pagination__link">' . ($page + 2) . '</a>';
        } else {
            $page2right = '';
        }
        if ($page + 1 <= $total_page) {
            $page1right = ' <a href="?page=' . ($page + 1) . $cat . '" class="pagination__link">' . ($page + 1) . '</a>';
        } else {
            $page1right = '';
        }
        if ($total_page > 1) {
            $page_nav_links = '&nbsp;&nbsp;&nbsp;' . $page5left . $page4left . $page3left . $page2left . $page1left . '<a href="#" class="pagination__link active">' . $page . '</a>' . $page1right . $page2right . $page3right . $page4right . $page5right . '&nbsp;&nbsp;&nbsp;';
        } else {
            $page_nav_links = '';
        }
