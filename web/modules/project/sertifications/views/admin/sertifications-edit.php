...
<div id="tabs">	
	...
	<div id="tabs-1">		
		...
		<div class="form_item">			
			<label for="old_price"><?= $text_old_price ?></label></br>			
			<input type="text" name="old_price" class="text" value="<?= $content['old_price'] ?>">		
		</div>				
		
		<div class="form_item">			
			<label for="price"><?= $text_price ?></label></br>			
			<input type="text" name="price" class="text" value="<?= $content['price'] ?>">		
		</div>				
		
		<div class="form_item">
			<label for="code"><?= $text_code ?></label></br>			
			<input type="text" name="code" class="text" value="<?= $content['code'] ?>">		
		</div>
		
		<div class="form_item_textarea">						
			<?php if(isset($languages)): ?>				
				<?php foreach ($languages as $item): ?>
					<label for="body" class="lang_img_<?= $item['lang_id']?>"><?= $text_body ?> </label>
					<?= $item['icon']?><textarea name="descriptions[<?= $item['lang_id']?>][body]" class="textarea_lang_<?= $item['lang_id']?>"><?= $content['descriptions'][$item['lang_id']]['body'] ?></textarea>				
				<?php endforeach; ?>			
			<?php else: ?>				
				<label for="body"><?= $text_body ?></label>
				<textarea name="descriptions[1][body]" class="lang_editor"><?= $content['descriptions'][1]['body'] ?></textarea>			
			<?php endif; ?>		
		</div>		
		...
		<div class="form_item" style="top:35px">			
			...
			<?php endif; ?>			
			<label for="status"><?= $text_sertifications_status ?></label>		
		</div>	
	</div>		
