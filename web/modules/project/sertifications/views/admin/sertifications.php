<? if (isset($errors)) { ?>
    ...
    <? } ?>
<? } ?>
<h2 class="title"><?= $text_sertifications ?> <a href="/admin/sertifications/add" title="<?= $text_add_new_sertifications ?>"><img src="/images/admin/add.png" style="margin-bottom:-10px;"></a></h2>
...
<?php if (isset($contents) AND count($contents) > 0): ?>
    <form method="post" action="/admin/multiaction/update" id="multiaction_form">
        <table>
            <thead>
                <tr>
                    <td><strong><?= $text_sertifications_thead_name ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
					<td><strong><?= $text_sertifications_thead_old_price ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
					<td><strong><?= $text_sertifications_thead_price ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
                    <td><strong><?= $text_sertifications_thead_alias ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
                    <td class="last"><strong><?= $text_sertifications_thead_status ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
                    ...
                    <td class="last"><strong><?= $text_sertifications_thead_action ?></strong></td>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($contents as $key => $value): ?>
                    <tr>
						...
						<td><input type="text" name="old_price[<?= $value['id'] ?>]" class="text" value="<?= $value['old_price'] ?>"></td>
						<td><input type="text" name="price[<?= $value['id'] ?>]" class="text" value="<?= $value['price'] ?>"></td>
                        ...
                        <td class="last"><a href="/admin/sertifications/edit/<?= $value['id'] ?>" class="edit"><?= $text_edit ?></a>&nbsp;&nbsp;<a href="/admin/sertifications/delete/<?= $value['id'] ?>" class="delete"><?= $text_delete_img ?></a></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>		
        </table>
        <input type="hidden" name="module" value="sertifications" />
    </form>
<?php else: ?>
