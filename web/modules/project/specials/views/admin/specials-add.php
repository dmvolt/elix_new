...
<h2 class="title"><?= $text_add_new_specials ?></h2>
...
<div id="tabs">
	...
	<div id="tabs-1">
		...
		<div class="form_item_textarea">
			<?php if(isset($languages)): ?>
				<?php foreach ($languages as $item): ?>
					<label for="teaser" class="lang_img_<?= $item['lang_id']?>">Должность </label><?= $item['icon']?><textarea name="descriptions[<?= $item['lang_id']?>][teaser]" class="textarea_lang_<?= $item['lang_id']?>"><?= $post['descriptions'][$item['lang_id']]['teaser'] ?></textarea>
				<?php endforeach; ?>
			<?php else: ?>
				<label for="teaser">Должность </label><textarea name="descriptions[1][teaser]" class="lang_editor"><?= $post['descriptions'][1]['teaser'] ?></textarea>
			<?php endif; ?>
		...
		<div class="form_item" style="top:35px">
			<input type="checkbox" id="status" name="status" checked="checked" value="1"><label for="status"><?= $text_specials_status ?></label>
		</div>
	</div>
