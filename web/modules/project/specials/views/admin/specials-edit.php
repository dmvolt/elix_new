	...
	<div id="tabs-1">
		...
		<div class="form_item_textarea">				
			<?php if(isset($languages)): ?>
				<?php foreach ($languages as $item): ?>
					<label for="teaser" class="lang_img_<?= $item['lang_id']?>">Должность </label><?= $item['icon']?><textarea name="descriptions[<?= $item['lang_id']?>][teaser]" class="textarea_lang_<?= $item['lang_id']?>"><?= $content['descriptions'][$item['lang_id']]['teaser'] ?></textarea>
				<?php endforeach; ?>
			<?php else: ?>			
				<label for="teaser">Должность </label><textarea name="descriptions[1][teaser]" class="lang_editor"><?= $content['descriptions'][1]['teaser'] ?></textarea></br>
			<?php endif; ?>
		</div>
		...
		<div class="form_item_textarea">			
			<?php if(isset($languages)): ?>
				...
				<?php endforeach; ?>
			<?php else: ?>
				<label for="body"><?= $text_body ?></label><textarea name="descriptions[1][body]" class="lang_editor"><?= $content['descriptions'][1]['body'] ?></textarea>
			<?php endif; ?>
		</div>
		...
		<div class="form_item" style="top:35px">
			...
			<?php endif; ?>
			<label for="status"><?= $text_specials_status ?></label>
		</div>
	</div>
