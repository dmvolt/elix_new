...
class Model_Cpages {
    ...
    public function add($data = array()) {
        DB::query(Database::INSERT, 'INSERT INTO ' . $this->tableName . ' (alias, in_front, weight, status) VALUES (:alias, :in_front, :weight, :status)')
                ->parameters(array(
    ...
    public function get_content($id = '') {
        ...
        if (count($result)>0){
			...
			$images = $file_obj->get_files_by_content_id($result[0]['id'], 'cpages', 1);
			$files = $file_obj->get_files_by_content_id($result[0]['id'], 'cpages', 0);
			if (empty($images)) {
				$filename = false;
			} else {
			...
			$edit_interface = Liteedit::get_interface($result[0]['id'], 'cpages', 'teaser', true);
    ...
    public function get_total($cat = 0, $is_adminka = 0, $lang_id = 1) {
		if($cat){
			if ($is_adminka) {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " s ON cc.content_id = s.id INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = s.id WHERE cd.lang_id = " . $lang_id . " AND cd.module = 'cpages' AND cc.category_id = " . $cat . " AND cc.module = 'cpages'";
			} else {
				$sql = "SELECT COUNT(id) AS total FROM `" . $this->tableName2 . "` cc INNER JOIN " . $this->tableName . " s ON cc.content_id = s.id INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = s.id WHERE cd.lang_id = " . $lang_id . " AND cd.module = 'cpages' AND cc.category_id = " . $cat . " AND cc.module = 'cpages' AND s.status = 1";
			}
		} else {
			if ($is_adminka) {
				$sql = "SELECT COUNT(id) AS total FROM " . $this->tableName . " s INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = s.id WHERE cd.lang_id = " . $lang_id . " AND cd.module = 'cpages'";
			} else {
				$sql = "SELECT COUNT(id) AS total FROM " . $this->tableName . " s INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = s.id WHERE cd.lang_id = " . $lang_id . " AND cd.module = 'cpages' AND s.status = 1";
			}
		}
