...
<h2 class="title"><?= $text_cpages ?> <a href="/admin/cpages/add<?= $parameters ?>" title="<?= $text_add_new_cpages ?>"><img src="/images/admin/add.png" style="margin-bottom:-10px;"></a></h2>
...
<?= $pagination ?>
    <form method="post" action="/admin/multiaction/update" id="multiaction_form">
        <table>
            <thead>
                <tr>
                    <td><strong><?= $text_cpages_thead_name ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
                    <td class="last"><strong><?= $text_cpages_thead_status ?></strong><br><a onclick="$('#multiaction_form').submit();" class="btn_core btn_core_blue btn_core_sm"><span><?= $text_save ?></span></a></td>
                    ...
                    <td class="last"><strong><?= $text_cpages_thead_action ?></strong></td>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($contents as $key => $value): ?>
                    <tr>
                        ...
                        <td class="last"><a href="/admin/cpages/edit/<?= $value['id'] ?><?= $parameters ?>" class="edit"><?= $text_edit ?></a>&nbsp;&nbsp;<a href="/admin/cpages/delete/<?= $value['id'] ?><?= $parameters ?>" class="delete"><?= $text_delete_img ?></a></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>		
        </table>
		<input type="hidden" name="parameters" value="<?= $parameters ?>" />
        <input type="hidden" name="module" value="cpages" />
    </form>
<?= $pagination2 ?>
<?php else: ?>
