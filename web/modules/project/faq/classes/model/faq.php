...
class Model_Faq {
    ...
    public function add($data = array()) {
        $result = DB::query(Database::INSERT, 'INSERT INTO ' . $this->tableName . ' (date, parent_id, group_id, weight, status) VALUES (:date, :parent_id, :group_id, :weight, :status)')
					->parameters(array(		
    ...
    public function get_all($is_adminka = 0, $start = 0, $num = 100, $field = 'f.id', $inner_join = '', $filter = '', $parent_id = 0, $lang_id = 1) {
		$contents = array();
        if ($is_adminka) {
            $sql = "SELECT * FROM " . $this->tableName . " f INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = f.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module AND f.parent_id = :parent_id " . $filter . " ORDER BY f.weight, :field DESC LIMIT :start, :num";
        } else {
            $sql = "SELECT * FROM " . $this->tableName . " f INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = f.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module AND f.parent_id = :parent_id AND f.status = 1 " . $filter . " ORDER BY f.weight, :field DESC LIMIT :start, :num";
        } 
	...
	public function get_total_all($is_adminka = 0, $inner_join = '', $filter = '', $lang_id = 1) {
		...
		if ($is_adminka) {
			$filter .= ' AND f.status = 1 ';
		}
		
		$query = DB::query(Database::SELECT, "SELECT COUNT(id) AS total FROM " . $this->tableName . " f INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = f.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module " . $filter)
					->parameters(array(
						':lang_id' => $lang_id,
						':module' => 'faq',
                    ))->execute();
