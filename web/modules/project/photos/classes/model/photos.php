...
class Model_Photos {
    ...
    public function get_all($is_adminka = 0, $start = 0, $num = 100, $field = 'p.id', $inner_join = '', $filter = '', $lang_id = 1) {
		$contents = array();
        if ($is_adminka) {
            $sql = "SELECT * FROM " . $this->tableName . " p INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = p.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module " . $filter . " ORDER BY p.weight, :field DESC LIMIT :start, :num";
        } else {
            $sql = "SELECT * FROM " . $this->tableName . " p INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = p.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module AND p.status = 1 " . $filter . " ORDER BY p.weight, :field DESC LIMIT :start, :num";
        } 
	...
	public function get_total_all($is_adminka = 0, $inner_join = '', $filter = '', $lang_id = 1) {
		...
		$query = DB::query(Database::SELECT, "SELECT COUNT(id) AS total FROM " . $this->tableName . " p INNER JOIN " . $this->tableDesc . " cd ON cd.content_id = p.id " . $inner_join . " WHERE cd.lang_id = :lang_id AND cd.module = :module " . $filter)
					->parameters(array(
						':lang_id' => $lang_id,
						':module' => 'photos',
                    ))->execute();
