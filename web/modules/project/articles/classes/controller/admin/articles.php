...
class Controller_Admin_Articles extends Controller_Admin_Template {
    ...
    public function action_index() {
		...
		if($catid2 AND !empty($catid2)){
			$filter_query .= ' AND (cc2.category_id = '.$catid2.' AND cc2.module = "articles") ';		
			$inner_join .= ' INNER JOIN `contents_categories` cc2 ON cc2.content_id = a.id ';
			
			$cat_name .= ($i)?', '.$cats[1][$catid2]['name']:'Статьи - '.$cats[1][$catid2]['name'];
    ...
    public function action_add() {
		...
		/********************* Операции с модулями ********************/
		$data['categories_form1'] = Controller_Admin_Categories::get_fields(array(), 'articles', 1);
    ...
    public function action_edit() {
		...
		/********************* Операции с модулями ********************/	
		$data['categories_form1'] = Controller_Admin_Categories::get_fields($data['content'], 'articles', 1);
