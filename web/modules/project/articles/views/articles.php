				...
				<?php foreach($articles as $article): ?>
					<div class="announce__entry">
						<?php if($article['thumb']): ?>
							<a href="<?= Data::_('lang_uri') . $cat_url . '/articles/' . $article['alias'] ?>" class="announce__a"><img src="<?= Im::imagepath('preview', $article['thumb']) ?>"></a> <!-- 230 X 230 -->
						<?php endif; ?>
						<div class="announce__text">
							<?= $article['edit_interface'] ?>
							<h3><a href="<?= Data::_('lang_uri') . $cat_url . '/articles/' . $article['alias'] ?>"><?= $article['descriptions'][Data::_('lang_id')]['title'] ?></a></h3>
							<div class="announce__date"><?= Text::format_date($article['date']) ?></div>
							<div id="articles_content_<?= $article['id'] ?>"><?= $article['descriptions'][Data::_('lang_id')]['teaser'] ?></div>
						</div>
						<div class="announce__footer"><a href="<?= Data::_('lang_uri') . $cat_url . '/articles/' . $article['alias'] ?>">Читать далее</a></div>
					</div>
				<?php endforeach; ?> 
			</div>
